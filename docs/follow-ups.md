...
- **MODE: LOCAL ONLY (current, 2026-05-19).** Testing is local docker-compose on the dev machine; **VPS deferred** (artifacts retained unchanged as the eventual VPS path). Runbook §L = local quickstart; VPS sections retained for later. All safety invariants apply locally (S5-1 guard, deploy-smoke gate, no API key, invite/adults-only).
- **Image build VERIFIED 2026-05-20** (`docker build -f Dockerfile.alpha`, Docker 29.2.0): npm ci + nest build + python deps + Claude CLI install + non-root all pass; no `.env` in context. Build revealed one small fix → removed `ENV ANTHROPIC_API_KEY=""` (unset is the correct state for OAuth-token auth precedence; also clears the buildkit `SecretsUsedInArgOrEnv` lint). Runtime defense unchanged: compose forces it empty, overriding `env_file`. Rebuild confirmed warning gone. No secrets/auth weakened.
